...
use App\Listeners\Kassenzone\KassenzonePodcastMapper;
 ...
 */
class ProcessKassenzonePodcastRssFeed
{
    /**
     * We need this prefix to make sure
     * this feed has its own number
     */
    const PREFIX_EXTERNAL_ID = 'kassenzone_podcast-';
     ...
     */
    const NAME_OF_FEED = 'kassenzone_podcasts';
    ...
    public function handle(ProcessNewsFeedsEvent $event): void
    {
        ...
        if ($xml !== null) {
            ...
            foreach ($xml->channel->item as $item) {
                $external_id = (string) self::PREFIX_EXTERNAL_ID . $item->guid;
     ...
     * @param $item
     *
     * @return KassenzonePodcastMapper
     */
    private function getKassenzoneArtikelMapper($item): KassenzonePodcastMapper
    {
        return new KassenzonePodcastMapper($item);
    }
     ...
     *
     * @param KassenzonePodcastMapper $mapper
     */
    private function saveNewLinks(KassenzonePodcastMapper $mapper): void
    {
